...
export default function MessagesScreen() {
    ...
    const loadAllMessages = () => {
        ...
        const unsubscribe1 = onSnapshot(q, (snapshot) => {
            ...
            updateAllMessages(sentMessages, 'sent');
        });
        ...
        const unsubscribe2 = onSnapshot(q2, (snapshot) => {
            ...
            updateAllMessages(receivedMessages, 'received');
        });
    ...
    const updateAllMessages = async (messages: Message[], type: 'sent' | 'received') => {
        // Add user data to messages
        const messagesWithUsers = await Promise.all(
            messages.map(async (message) => {
                const otherUserId = type === 'sent' ? message.recipientId : message.senderId;
                ...
                if (!otherUser) {
                    try {
                        ...
                        return userData ? { ...message, otherUser: userData } as MessageWithUser : null;
                    } catch (error) {
                        return null;
                    }
                }
                
                return { ...message, otherUser } as MessageWithUser;
            })
        );

        const validMessages = messagesWithUsers.filter(Boolean) as MessageWithUser[];
        
        // Merge with existing messages and sort by recency
        setAllMessages(prev => {
            const filtered = prev.filter(m => 
                type === 'sent' ? m.senderId !== auth.currentUser?.uid : m.recipientId !== auth.currentUser?.uid
            );
            const combined = [...filtered, ...validMessages];
    ...
    const loadUserMessages = (userId: string) => {
        ...
        const q = query(
            ...
            where('recipientId', 'in', [auth.currentUser.uid, userId]),
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const messages = snapshot.docs
                ...
                .filter(message => 
                    (message.senderId === auth.currentUser?.uid && message.recipientId === userId) ||
                    (message.senderId === userId && message.recipientId === auth.currentUser?.uid)
                );
